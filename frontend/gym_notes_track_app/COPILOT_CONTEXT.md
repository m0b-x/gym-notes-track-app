...
- Syntax (line-led, optional leading whitespace): `$= 100` set/checkpoint, `$+ 12.50 label` add, `$- 8 label` subtract, `$* 1.19` multiply, `$/ 2` divide, `$$ label` running total, `$? label` net change since the last `$=` (or note start), `$! 500 label` spending target (inert; its displayed value is the remaining budget = target − spent since the anchor, via `MarkdownMoneySyntax.displayValue` — the single source of truth for stored entry values across all three folds). `$=` doubles as a period boundary — put one under a month/year heading and close the period with `$?`. Target rows render the remaining as a sign-colored pill in the preview and stay source-faithful (`!` → `◎`, text-keyed cache) in the editor.
...
- **Preview**: `LineBasedMarkdownBuilder._computeMoneyLedger()` folds the ledger once per `prepare` (O(n) lead-char probe over `_source`; only `$`-led lines outside multi-line blocks parse). `_buildMoneyLine` consumes the `$x` marker as chrome (like `[!TYPE]`), renders sign glyphs `+ − × ÷ = Σ Δ` in the shared `MarkdownConstants.moneyPositive/moneyNegative/moneyNeutral` palette, keeps the amount and label offset-mapped for search, and appends the computed running balance (dimmed on op rows, pill-tinted on `$$`/`$?` rows). Computed values are not source text and carry no offsets.
- **Editor**: op lines are purely textual (conceal `$`, substitute `-*/` 1:1 with `−×÷`, tint amount; text-keyed span cache). `$$`/`$?` lines are positional: the balance comes from `MarkdownEditorLineIndex.moneyValueAt` (an incremental money pass mirroring the task pass — per-segment entry balance + anchor snapshots, truncate-and-append results, ~one segment rescan per keystroke) and styles through the positional memo with the value folded into the key (`'m:$value:$text'`). The value paints via `_EditorMoneyTotalSpan` (fork `CodeInlinePaintSpan`, substitutes 1:1 for the second marker char, height under the strut). Reveal lines show raw dimmed markers and skip the paint.
...
- Toolbar: seven default shortcuts — `default_money` (`$+ `, guard `money_shortcut_seeded`) plus subtract/multiply/divide/set/total/change siblings seeded as one batch via `_seedDefaultShortcutBatchIfNeeded` (guard `money_ops_shortcuts_seeded`).
...
- **Tap-to-detail**: `$$`/`$?` values are tappable on both surfaces (preview pill recognizer keyed `'money:$lineIndex'`; editor via `ModernEditorWrapper._resolveTapAction` zone `[markerStart, labelStart)` after the shared pass-through guards, enabled through `onMoneyTap` in the interceptor condition). Both route to the page's `_handleMoneyTap` → `MarkdownMoneySyntax.collectEntries` (fence-aware via `lineInFence`, O(document) on tap only) → `MoneyDetailSheet`. `$?` sheets list entries since the `$=` anchor only.
