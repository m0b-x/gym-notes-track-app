...
enum MarkdownBlockKind { codeFence }
...
class MarkdownChunker {
  ...
  /// Scans the source once for multi-line blocks. Today that is fenced
  /// code (```), including an unterminated fence that runs to the end of
  /// the document. Fences are non-atomic (splittable) so virtualization
  /// of very large code blocks is preserved.
  static List<MarkdownBlock> _scanBlocks(
    ...
    for (int i = 0; i < lineCount; i++) {
      final isFence = lineAt(i).trimLeft().startsWith('```');
      if (!isFence) continue;
      if (!inFence) {
        inFence = true;
        fenceStart = i;
